<script lang="ts">
  ...
  <main class="h-full">
    <div class="w-full flex flex-col items-center justify-center bg-orange-600 {$display.isDesktop ? "" : "w-full p-4"}">
      <MainLogo type="alt" />
    </div>
    <div class="{$display.isDesktop ? "p-6 mx-20" : "p-4 mx-4"}">
        <h1 class="font-bold my-6 {$display.isDesktop ? "text-xl" : "text-lg"}">Live Waitlist</h1>
        {#if emails.length === 0}
          <p>No emails found or loading...</p>
        {:else}
          <ul class=" p-4 bg-gray-100 rounded-lg {$display.isDesktop ? "space-y-4 text-sm" : "space-y-2 text-xs"}">
