<script lang="ts">
  ...
  <main class="flex flex-col items-center justify-center {$display.isDesktop ? "" : "text-sm"}" id="join-waitlist">
    <div class="w-full flex flex-col items-center justify-center bg-orange-600 {$display.isDesktop ? "" : "p-4"}">
        ...
        <form
        class="space-x-2 my-4 flex items-center justify-center {$display.isDesktop ? "w-1/3" : ""}"
        on:submit|preventDefault={submitEmail}>
        <input
            type="email"
            class="input rounded-md {$display.isDesktop? "" : "text-xs"}"
            bind:value={email}
            placeholder="Enter your email"
            required
        />
        <button type="submit" class="btn variant-filled rounded-md {$display.isDesktop ? "" : "text-xs"}" disabled={submitting}>Submit</button>
        </form>
    </div>
    <div class="container text-md {$display.isDesktop ? "p-10 px-40" : "p-4 m-4"}">
        <p class="">
        ...
        <h2 class="font-bold {$display.isDesktop ? "my-4" : "mt-4"}">A New Era of Secure and Personalized Trading</h2>
        <p class="">
        ...
        <h2 class="font-bold {$display.isDesktop ? "my-4" : "mt-4"}">Empowering Communities and Creating Opportunities</h2>
        ...
        <h2 class="font-bold {$display.isDesktop ? "my-4" : "mt-4"}">Why Join <StylizedName />?</h2>
        <ul class="list-disc {$display.isDesktop ? "space-y-4 my-4" : "space-y-2 m-4"}">
